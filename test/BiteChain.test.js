...
contract('TestBiteChain', function(accounts) {
  ...
  it("Pulling Menu info", async () => {
    ...
    assert.equal(getMenuLength, 3, 'should get the correct number of items on the menu');
  ...
  it("Customer making an order", async () => {
    const restaurant = await BiteChain.deployed();
    await restaurant.customerSubmitOrder("1","1","","",{from: customer,value:1});

    // Grabs orderID and qty of orders on record
    let custGetOrder = await restaurant.customerGetOpenOrders(0,{from: customer});

    // Get customer orders, check that qty is greater than 0    
    let orderExists = false;
    if (custGetOrder[1] > 0) {orderExists = true;}
    assert.equal(orderExists, true, 'should return true if there is at least 1 order');
  });
  ...
  it("Check roles of workers", async () => {
    ...
    await restaurant.customerSubmitOrder("1","1","","",{from: customer,value:1});
    let custGetOrder = await restaurant.customerGetOpenOrders(0,{from: customer});
    let status = await restaurant.getOrderStatus(custGetOrder[0],{from: customer});
    ...
    status = await restaurant.getOrderStatus(custGetOrder[0],{from: customer});
  ...
  it("Owner withdrawl funds", async () => {
    const bank = await BiteChain.deployed();

    // Check that owner can withdrawl
